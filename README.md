...
the data, if you don't want all of it. 

Index annotations are a mechanism for one organization to create a 
database table that can be joined to Common Crawl's columnar url 
index or host index. Then they or another organization can use the 
resulting joined index.

As an example, perhaps you're interested in content on the web that
has a Creative Commons license ( currently less than 0.1% of all of 
the content on the web.) In this scenario, you'd like to use a
list of webpages to extract just the CC licensed content. 
